...
public class FixedGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void setTotalColumnCount(int count) {
        ...
        //Thomas_question_do_we_really_need_requestinglayout
        requestLayout();
    }
    ...
    private void fillGrid(int direction, int emptyLeft, int emptyTop,
        ...
        int startLeftOffset = getPaddingLeft() + emptyLeft;//recyclerView的paddingleft
        int startTopOffset = getPaddingTop() + emptyTop;//recyclerView 的paddingTop

        int childCount = getChildCount();//已经attached到recyclerView的子view
        if (getChildCount() != 0) {
        ...
        for (int i = 0; i < getVisibleChildCount(); i++) {
            ...
            if (view == null) {
                ...
                if (!state.isPreLayout()) {
                    ...
                    Log.d(TAG,"pos i view [row,column]:  " + "["+lp.row+","+lp.column+"]");
                }
                ...
                Log.d(TAG, "pos i view [left,top,right,bottom]: " + "[" + left+"," + top+ "," +
                        right+"," + bottom+"]");
